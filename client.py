...
class Client(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.keyLineEdit.setText("easykey")          # Ключ по умолчанию
    ...
    def on_ready_read(self):
        ...
        if match:
            ...
            # Сообщение
            message = match.group(3)[1:]
        else:
            self.handle_server_message('Не удалось расшифровать сообщение')

        key = self.keyLineEdit.text()

        # Выбираем первые 8 байт
        key = key[:8]

        # Если ключ короче 8 байт, дополняем его пробелами
        if len(key) < 8:
            key += ' ' * (8 - len(key))
        secureCommunication = base64.b64decode(message)
        # Получаем текст из поля ввода сообщения и raZшифровываем
        message = decrypt(message, key)
        seed = int(hashlib.sha256(secureCommunication).hexdigest(), 16)
        random.seed(seed)
        if not message:
            message = ''.join(random.choice(string.ascii_letters + string.digits + string.punctuation + ' ') for _ in range(len(obmanka_simence)))
